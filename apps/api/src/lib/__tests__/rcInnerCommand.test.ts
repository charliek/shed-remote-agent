...
describe('buildInnerCommand', () => {
  it('agent uses claude remote-control with --spawn same-dir', () => {
    expect(buildInnerCommand('agent', 'my-shed/abc123')).toBe(
      'claude remote-control --name my-shed/abc123 --spawn same-dir',
    );
  });

  it('repl runs interactive claude with /rc as initial argv', () => {
    expect(buildInnerCommand('repl', 'my-shed/abc123')).toBe('claude --name my-shed/abc123 /rc');
  });
  ...
  it('quotes display names with spaces', () => {
    expect(buildInnerCommand('agent', 'Friday Bug Fix')).toBe(
      "claude remote-control --name 'Friday Bug Fix' --spawn same-dir",
    );
    expect(buildInnerCommand('repl', 'Friday Bug Fix')).toBe("claude --name 'Friday Bug Fix' /rc");
  });

  it('quotes display names with single quotes via POSIX backslash trick', () => {
    expect(buildInnerCommand('agent', "it's mine")).toBe(
